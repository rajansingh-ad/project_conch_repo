...
CONFIG_FILE = os.environ.get(
    "CONFIG_FILE",
    "/mnt/fss-rodn-iad-odi-fs/POC/config/nike_bom_usage_blob_loader.conf"
)

DEFAULT_INPUT_DIR = "/mnt/fss-rodn-iad-odi-fs/POC/Input/Nike"
DEFAULT_TARGET_TABLE = "CUSTOM_STG_ADW.DM_STG_BOM_USAGE"
...
FILE_CONFIGS = [
    {
        ...
        "lob": "Apparel",
        "file_type": "BOM Usage",
        "source_kind": "zip_inner_bom_file",
        "allow_zip": True,
        "sheet_keywords": ["bom usage", "bom_usage", "bomusage", "bom"],
        "date_rule": "first"
    },
    {
...
for subfolder in ["/Apparel", "/Footwear", "/Lion_Brothers"]:
    if INPUT_DIR.rstrip("/").endswith(subfolder):
...
def extract_date_candidates(file_name):
    ...
    # 20260427 / 2026_04_27 / 2026-04-27
    ...
    # 27042026 / 27_04_2026 / 27-04-2026
...
def choose_file_creation_date(file_name, date_rule):
    ...
    if len(candidates) == 0:
        print("WARNING: No date found in filename. Using current date for: " + os.path.basename(file_name))
...
def clean_value_for_column(column_name, value):
...
def detect_header_row(raw_df):
    ...
    for idx in range(scan_limit):
        ...
        for value in row:
            ...
            if text != "":
                non_empty_count += 1
                joined_values.append(text.lower())

        row_text = " ".join(joined_values)

        for keyword in HEADER_KEYWORDS:
            if keyword in row_text:
                keyword_score += 20

        score = keyword_score + non_empty_count
...
def select_excel_sheet(file_path, config):
    ...
    raise Exception(
        "Required BOM Usage sheet not found. File: "
...
def read_excel_sheet_as_raw_dataframe(file_path, selected_sheet):
    lower_file = file_path.lower()
...
def read_excel_source(file_path, config):
    ...
    raw_df = read_excel_sheet_as_raw_dataframe(file_path, selected_sheet)
...
def is_bom_usage_file_name(file_name):
    ...
    return (
        ...
        or "bomusage" in lower_file
    )


def is_supported_source_file(file_name, allow_zip):
...
def get_files_for_config(config):
    ...
    for file_name in os.listdir(folder_path):
        ...
        if not is_supported_source_file(file_name, config.get("allow_zip", False)):
            continue

        if config.get("source_kind") == "zip_inner_bom_file" and not file_name.lower().endswith(".zip"):
            continue

        if config.get("source_kind") == "excel_bom_sheet" and file_name.lower().endswith(".zip"):
            continue
    ...
    if len(candidates) == 0:
        print("No BOM Usage source files found in folder: " + folder_path)
        return []
    ...
    print("Available BOM Usage source files for " + config["lob"] + " / " + config["file_type"] + ":")
...
def is_target_apparel_bom_member(member):
    lower_member = os.path.basename(member).lower()

    if member.endswith("/"):
        return False

    if lower_member.startswith("~$"):
        return False

    if not lower_member.endswith((".xlsx", ".xls", ".xlsm")):
        return False

    return is_bom_usage_file_name(lower_member)
...
def insert_dataframe_as_blob(conn, cursor, df, config, source_file_name, file_creation_date):
    ...
    for values in df.itertuples(index=False, name=None):
        ...
        for i, col in enumerate(columns):
            row_payload[col] = clean_value_for_column(col, values[i])
...
def process_excel_file(conn, cursor, file_path, config, source_file_name, file_creation_date):
    print("--------------------------------------------------")
    print("Processing Nike BOM Usage file:")
...
def process_apparel_zip(conn, cursor, zip_path, config, file_creation_date):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            target_members = [
                ...
                if is_target_apparel_bom_member(member)
            ]

            if len(target_members) == 0:
                print("WARNING: No BOM Usage Excel file found inside Apparel ZIP. Skipping ZIP: " + zip_file_name)
                return 0

            print("Target BOM Usage files inside Apparel ZIP:")
    ...
    print("Total Apparel ZIP BOM Usage processing seconds: " + str(zip_seconds))
    return total_inserted
...
def process_config(conn, cursor, config):
    ...
    for selected_file in selected_files:
        print("==================================================")
        print("Selected source for " + config["lob"] + " / " + config["file_type"] + ":")
        print("  " + selected_file["path"])
        print("  FileCreationDate=" + selected_file["file_creation_date"])

        if config.get("source_kind") == "zip_inner_bom_file":
            config_total += process_apparel_zip(
                conn=conn,
                cursor=cursor,
...
def print_runtime_config():
    print("==================================================")
    print("Nike BOM Usage BLOB Loader")
...
def main():
    ...
    try:
        ...
        print("==================================================")
        print("Nike BOM Usage BLOB load completed.")
